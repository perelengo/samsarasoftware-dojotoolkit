	 ...
	 return declare('samsarasoftware/ws/WSDL2Client',null, {
		...
		processForm: function (endpoint,description,serviceDescription){
			...
			for(var operationName in endpoint.binding.operations){
					...
					if(endpoint.binding.type=="http://www.w3.org/ns/wsdl/http"){
						...
						//No forms are generated as stubs are enough
					}else{
		...
		_executeJsonRpcMethod: function (method,endpoint){
			...
			if(endpoint.binding["wjsonrpc:protocol"]=="http://www.w3.org/ns/wsdl/http"){
				...
				//FIXME
				throw new Error("Only wjsonrpc:protocol=\"http://www.w3.org/ns/wsdl/http\" is supported actually");
			}
			
			var deserializeFunc=function(method,endpoint,results){
				return request._envDef.deserialize.call(this,[method, endpoint,results]);
			};
			promise.then(lang.partial(deserializeFunc, method), lang.partial(deserializeFunc, method, endpoint));
			return promise;
		},
		...
		_getJsonRpcRequest: function _getRequest(method,endpoint,args){
			...
			var envDef = samsarasoftware.jsonrpc.envelopeRegistry.match(protocolVersion);
			...
			request._envDef = envDef;// save this for executeMethod
				...
				if((args.length==1) && dojo.isObject(args[0])){
					...
					for(i=0;i<methodParamsXsd.length;i++){
						//if(typeof args[i] != "undefined" || !method.params[i].optional){
							data[self.removeNamespace(methodParamsXsd[i].attributes["name"].value)]=args[i];
						//}
					}
					args = data;
				}
				...
				for(i=0;i<methodParamsXsd.length;i++){
					...
					if(param.attributes["minOccurs"] && param.attributes["minOccurs"].value!="0" && !args[paramName]){
						if(param.attributes["default"]){
							args[paramName] = param.attributes["default"].value;
			...
			var out= dojo.mixin(request, {
				...
				,sync: (endpoint._options.sync!=null || samsarasoftware.ws.sync!=null )?(endpoint._options.sync || samsarasoftware.ws.sync):true
		...
		_executeMethod: function (method,endpoint){
			...
			promise.then(lang.partial(deserializeFunc, method,endpoint), lang.partial(deserializeFunc, method, endpoint));
			return promise;
		},
		...
		_getRequest2: function (method,endpoint, args){
			// the serializer is expecting named params
			var args2=new Array();
			var methodParamsXsd=self.getTypeHierarchyAttributes(method.messages["wsdl:input"][0].type,self.description);
		...
		processOperationMessages: function (operation,description){
			...
			for(i=0;i<operation.childNodes.length;i++){
				...
				var nsResolver2 = document.createNSResolver( description.ownerDocument == null ? description.documentElement : description.ownerDocument.documentElement );
				var xpathResultIterator2 = document.evaluate( "//xs:element[@name='"+elementName+"']", description, nsResolver2, XPathResult.ORDERED_NODE_ITERATOR_TYPE, null );
				var elementReference = xpathResultIterator2.iterateNext();
  
				if(elementReference==null) throw new Error("Cannot find element with name "+elementName);
				if(xpathResultIterator2.iterateNext()!=null)  throw new Error("Found more than one element with name "+elementName);
				...
				if(typeReference==null) throw new Error("Cannot find type with name "+typeName);
				if(xpathResultIterator4.iterateNext()!=null)  throw new Error("Found more than one type with name "+typeName);
...
//////////////////////////////////////			
require(["dojox/rpc/Service","dojo/AdapterRegistry", "dojo/request/xhr","dojo/request/handlers","samsarasoftware/rpc/JsonRpc2EnvelopeHandler"],function(Service, AdapterRegistry, xhr, handlers,JsonRpc2EnvelopeHandler){
	...
	samsarasoftware.ws.envelopeRegistry.register(
		...
		function(str){ return str == "http://www.w3.org/ns/wsdl/style/iri"; },
		{
			serialize:function( endpoint,method, data ){
				...
				var headers= {};
				headers["Content-type"]=method["whttp:inputSerialization"];
				headers["Accept"]=method["whttp:outputSerialization"]+";q=1, "+method["whttp:faultSerialization"]+";q=0.5";
			...
			deserialize:function(res){
				...
				if(endpoint._options.deserializerRegistry){
					return endpoint._options.deserializerRegistry.match(simplifiedContentType).deserialize(responseContentType, response.data);
				}else{
					try{
						return samsarasoftware.ws.deserializerRegistry.match(simplifiedContentType).deserialize(responseContentType, response.data);
					}catch(e){
		...
		if(method.style=="http://www.w3.org/ns/wsdl/style/iri"){
				...
				for(var i=0;i<map.length;i++){
					for(arg in map[i]){
						...
						//FIXME si es un array, no queda clar com fer-ho
						if(location.indexOf("{"+arg+"}")==-1 && method["whttp:ignoreUncited"]=="false")
							notFound[arg]=map[arg];
						else{
							var val="";
				...
				//con los not found hay que hacer:
