					...
					fetch: function(request,initialize){
						...
						if(this.srcNodeRef.memory==null){
								...
								if(initialize==undefined || (initialize!=undefined && initialize)){
									...
									var method=self.endpoint.binding.operations[self.operationName];
									this.service=function(req){
										var d=new Deferred();
										
										d.addCallback=function(f){
											d.__okCallback=f;
											d.then(d.__okCallback,d.__errCallback);
										}
										d.addErrback=function(f){
											d.__errCallback=f;
											d.then(d.__okCallback,d.__errCallback);
										}
										self.endpoint.stub[self.operationName](req).then(
											function (res){d.resolve(res);}
											,function (err){d.cancel(err);}
										);
										return d;
									}
