...
define([
	...
	function(declare,Registry,TextBox,RadioButton,DateTextBox,NumberTextBox,Textarea,CheckBox,Select,dom,dom_form,lang,DeferredList,Deferred,ValidationTextBox,FilteringSelect,all){	
		...
		function FormManagerClass() { return this; };
				// private functions
		FormManagerClass.prototype.exclude = "submit|image|reset|button";

				

		FormManagerClass.prototype.setValue=function (/* Object */ obj, /* String */ name, /* String */ value){
				// Modified version of dojo/dom-form.js by Pere Joseph
				// Rodriguez
				// Copyright (c) 2005-2016, The Dojo Foundation
				// All rights reserved.
		        
				// summary:
		        // For the named property in object, set the value. If a
				// value
		        // already exists and it is a string, convert the value to
				// be an
		        // array of values.
	    ...
	    FormManagerClass.prototype.getDataURIByteString= function(dataURI){
			var byteString;
		...
		FormManagerClass.prototype.getDataURIByteArray= function(dataURI){
		...
		FormManagerClass.prototype.getDataURIMimeType= function(dataURI){
			return dataURI.split(',')[0].split(':')[1].split(';')[0];
		};
		
		FormManagerClass.prototype.getBlob= function(dataURI) {
			try{
		...
		FormManagerClass.prototype.getBlobURL= function(blob) {
			try{
		...
		FormManagerClass.prototype.jsonToForm= function (form, inputData, reset){
			...
			try{
				for(var i = 0; i < form.domNode.elements.length; i++){
					...
					// if(elem.get('value')) elem.set('value','');
					// //afecta al filteringselect combinado
				...
				for(var i = 0; i < form.domNode.elements.length; i++){
					...
					// en orden de subclase a superclase, primero las
					// subclases
					if(elem.name==key && elem instanceof DateTextBox){
						...
						if(!(data==null)){ // si llega valor de la
											// petici?n
							var tempEl = elem;
							var data2=data;
							// preparamos la invocaci?n para buscar el
							// item del select con el id recibido en la
							// petici?n
							// para seleccionar un elemento del select
							// es necesario el item, no se puede hacer
							// con el id
							// var
							// item=(elem.store.get)?elem.store.fetchItemByIdentity({identity:data2}):elem.store.get(data2);
							...
							var createQuery=function (dataId, selectElement, retryCounter,def){
								return {
									query:      {'id':dataId}, // id
																...
																// a
																// buscar
									onComplete: function(item){ // funci?n
																// de
																// callback
																// una
																// vez
										...
										if (item.length == 0 ){ // si no
											...
											// en la lista, lo asigna
											selectElement.disabled = false;
											
											// el ItemFileReadStore
											// devuelve items diferentes
											// al service store, y para
											// que funcione
											// necesitamos pasar el
											// label al set('item'), o
											// lo ir? a pedir al service
											// store v?a getValue
											// ?ste devolver? un label
											// de tipo array, y el
											// textbox no lo sabr?
											// interpretar por ser un
											// array y no un string y
											// mostrar? el id en vez del
											// label.
											selectElement.set('item',null,false);
							...
							if(!elem.type || elem.type!="file"){
								...
								// var blob = new Blob([data[key].data],
					...
					// return a single promise that waits for the
					// execution of all its contained promises
		...
		FormManagerClass.prototype._formToJsonWithFilesAsDataUriExtended= function(formNode,callback, asArrayResult){
			// Modified version of dojo/dom-form.js by Pere Joseph
			// Rodriguez
			// Copyright (c) 2005-2016, The Dojo Foundation
			// All rights reserved.
			...
			// string properties in an object which it then returns.
			// Disabled form
			// elements, buttons, and other non-value form elements are
			// skipped.
			...
			// | <input type="text" name="no_value" value="blah"
			// disabled>
			...
			// preparamos la llamada al callback en el contexto de
			// window
			var callArgs=new Array();
			for(var i=2;i<arguments.length;i++) callArgs.push(arguments[i]);
			
			// Preparamos un defered list para llamar al callback cuando
			// todos los campos hayan sido cargados
				...
				for(var i = 0, l = elems.length; i < l; ++i){
					...
					if(_in && type && this.exclude.indexOf(type) < 0 && !item.disabled){
						if(_in && type=="file" && !item.disabled){
							...
							var callContext=new callContextFunc();
							fileCounter++;
							...
							if((type == "radio" || type == "checkbox") && val==null){
								val="false"; // Los checkbox son
												// true/false, no
												// permitimos null
							}
							if(val!="")
								this.setValue(ret, _in, val);
							if(type == "image"){
		...
		FormManagerClass.prototype.formToJsonWithFilesAsDataUri= function(formNode,callback){
			return this._formToJsonWithFilesAsDataUriExtended(formNode, callback, false);
		};
		FormManagerClass.prototype.formToJsonArrayWithFilesAsDataUri= function(formNode,callback){
			return this._formToJsonWithFilesAsDataUriExtended(formNode, callback, true);
		};	
		
		FormManagerClass.prototype.isValid= function (formId){
			return dijit.byId(formId).validate();
		};
		
		FormManagerClass.prototype.recursiveFillTemplate= function(content,templateElem,targetElem,results,row){
		...
		FormManagerClass.prototype.recursiveTemplateToJson= function(rowElement,obj){
		...
		FormManagerClass.prototype.jsonToTemplate= function(templateElem,targetElem,results){
			// reset content
			...
			// Returns a blended object that is an array of the
			// instantiated objects, but also can include a promise that
			// is resolved with the instantiated objects. This is done
			// for backwards compatibility. If the parser auto-requires
			// modules, it will always behave in a promise fashion and
			// parser.parse().then(function(instances){...}) should be
			// used.
			return require("dojo/parser").parse(targetElem);
		};
		
		FormManagerClass.prototype.setLastSelectedRows= function(tableId){
			var registry=require(["dijit/registry"]);	
			var grid=dijit.byId(tableId);
			if(grid.select.row.getSelected().length<1){
			}else{
				var ids=grid.select.row.getSelected() // Array of
														// selected Ids
		...
		FormManagerClass.prototype.getLastSupplierName= function(tableIdArrayOrString){
		...
		FormManagerClass.prototype.getLastSelectedRows= function(tableIdArrayOrString){
			...
			if(ids==""){
				...
				if(items.findIndex(i=> i=== undefined)!=-1){
					if(window.I18n){
						throw new Error("$i18n.mustSelectItem$");
					}else{
						throw new Error("Debe seleccionar algún registro.");
		...
		FormManagerClass.prototype.getItemSingleValue = function(item){
		...
		FormManagerClass.prototype.saveFormExtendedProperty= function(controller,formName,propertyName){
		...
		FormManager=new FormManagerClass();
		return FormManager;
